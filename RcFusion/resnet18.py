...
class ResNet(object):
    ...
    def _create_model(self):
        ...
        # Initial conv & pool
        self.conv1 = conv2d_fixed_padding(self.x, filter_size=[7, 7], num_filters=64, strides=2, name='conv1',
                                          add_bias=False)
        ...
        # Residual blocks (x4)
        self.res2, self.inter_res2 = residual_block(self.pool1, num_filters=64, strides=1, is_training=self.is_training,
                                                    name='2')
        self.res3, self.inter_res3 = residual_block(self.res2, num_filters=128, strides=2, is_training=self.is_training,
                                                    name='3')
        self.res4, self.inter_res4 = residual_block(self.res3, num_filters=256, strides=2, is_training=self.is_training,
                                                    name='4')
        self.res5, self.inter_res5 = residual_block(self.res4, num_filters=512, strides=2, is_training=self.is_training,
                                                    name='5')

        # Final pool & classifier
        self.pool2 = tf.nn.avg_pool(self.res5, ksize=[1, 7, 7, 1], strides=[1, 1, 1, 1], name='pool2', padding='VALID')
