...
def batch_normalization(x, is_training, name):
    return tf.layers.batch_normalization(x, axis=-1, momentum=0.99, epsilon=1e-3, center=True, scale=True,
...
def residual_block(x, num_filters, strides, is_training, name):
    ## Block A
    # branch2a
    conv_a2a = conv2d_fixed_padding(x, filter_size=[3, 3], num_filters=num_filters, strides=strides,
    ...
    # branch2b
    conv_a2b = conv2d_fixed_padding(relu_a2a, filter_size=[3, 3], num_filters=num_filters, strides=1,
                                    name='res' + name + 'a_branch2b', add_bias=False)
    ...
    # branch1
    conv_a1 = conv2d_fixed_padding(x, filter_size=[1, 1], num_filters=num_filters, strides=strides,
                                   name='res' + name + 'a_branch1', add_bias=False)
    ...
    ##Block B
    # branch2a
    conv_b2a = conv2d_fixed_padding(relu_add1, filter_size=[3, 3], num_filters=num_filters, strides=1,
                                    name='res' + name + 'b_branch2a', add_bias=False)
    ...
    # branch2b
    conv_b2b = conv2d_fixed_padding(relu_b2a, filter_size=[3, 3], num_filters=num_filters, strides=1,
                                    name='res' + name + 'b_branch2b', add_bias=False)
...
def bottleneck_block_up(x, num_filters, strides, is_training, name, character):
    ## Block A
    # branch2a
    conv_a2a = conv2d_fixed_padding(x, filter_size=[1, 1], num_filters=num_filters, strides=strides,
                                    name='res' + name + character + '_branch2a', add_bias=False)
    ...
    # branch2b
    conv_a2b = conv2d_fixed_padding(relu_a2a, filter_size=[3, 3], num_filters=num_filters, strides=1,
                                    name='res' + name + character + '_branch2b', add_bias=False)
    ...
    # branch2c
    conv_a2c = conv2d_fixed_padding(relu_a2b, filter_size=[1, 1], num_filters=num_filters * 4, strides=1,
                                    name='res' + name + character + '_branch2c', add_bias=False)
    bn_a2c = batch_normalization(conv_a2c, is_training=is_training, name='bn' + name + character + '_branch2c')

    # branch1
    conv_a1 = conv2d_fixed_padding(x, filter_size=[1, 1], num_filters=num_filters * 4, strides=strides,
                                   name='res' + name + character + '_branch1', add_bias=False)
...
def bottleneck_block_bottom(x, num_filters, strides, is_training, name, character):
    ## Block A
    # branch2a
    conv_a2a = conv2d_fixed_padding(x, filter_size=[1, 1], num_filters=num_filters, strides=1,
                                    name='res' + name + character + '_branch2a', add_bias=False)
    bn_a2a = batch_normalization(conv_a2a, is_training=is_training, name='bn' + name + character + '_branch2a')
    relu_a2a = tf.nn.relu(bn_a2a, name='res' + name + character + '_branch2a_relu')
...
def bottleneck_block(x, num_filters, strides, is_training, name):
    if (name == "2"):
        print("2")
        x_a = bottleneck_block_up(x, num_filters=num_filters, strides=strides, name=name, character='a',
                                  is_training=is_training)
        x_b = bottleneck_block_bottom(x_a, num_filters=num_filters, strides=strides, name=name, character='b',
                                      is_training=is_training)
        x_c = bottleneck_block_bottom(x_b, num_filters=num_filters, strides=strides, name=name, character='c',
                                      is_training=is_training)
    ...
    if (name == "3"):
        ...
        x_c = bottleneck_block_bottom(x_b, num_filters=num_filters, strides=strides, name=name, character='c',
                                      is_training=is_training)
        x_d = bottleneck_block_bottom(x_c, num_filters=num_filters, strides=strides, name=name, character='d',
                                      is_training=is_training)
    ...
    if (name == "4"):
        ...
        x_d = bottleneck_block_bottom(x_c, num_filters=num_filters, strides=strides, name=name, character='d',
                                      is_training=is_training)
        x_e = bottleneck_block_bottom(x_d, num_filters=num_filters, strides=strides, name=name, character='e',
                                      is_training=is_training)
        x_f = bottleneck_block_bottom(x_e, num_filters=num_filters, strides=strides, name=name, character='f',
                                      is_training=is_training)
...
def transformation_block(x, num_filters, kernel_constraint, training_phase, name):
    ...
    relu_conv_depth = tf.layers.max_pooling2d(relu_conv_depth, [kernel_size, kernel_size],
                                              strides=[kernel_size, kernel_size], name='max_' + name + '_depth')
...
def transformation_block_v1(x, num_filters, kernel_constraint, training_phase, name):
    ...
    # conv_spatial = tf.layers.conv2d(pool_spatial, depth_size, [kernel_size, kernel_size], strides=[kernel_size, kernel_size],
    #                                kernel_constraint=kernel_constraint, name=name+'_spatial', use_bias=False)
    bn_conv_spatial = tf.layers.batch_normalization(conv_spatial, training=training_phase,
                                                    name='bn_' + name + '_spatial')
...
def transformation_block_v1_bis(x, num_filters, kernel_constraint, training_phase, name):
    ...
    sep_conv = separable_conv2d(x, [7, 7, int(x.get_shape()[-1]), 1], [1, 1, int(x.get_shape()[-1]), num_filters],
                                [1, 7, 7, 1], name='sep_' + name)
    ...
    out = tf.layers.max_pooling2d(relu_sep_conv, [kernel_size, kernel_size], strides=[kernel_size, kernel_size],
                                  name='avg_pool__' + name)
...
def transformation_block_v3(x, num_filters, kernel_constraint, training_phase, name):
    ...
    if max_pool_stride <= 1:
        ...
        x_pool = tf.layers.max_pooling2d(x, [max_pool_stride + 1, max_pool_stride + 1],
                                         strides=[max_pool_stride, max_pool_stride], name='pool_' + name)
    print(x.get_shape(), num_filters)
    print(striding_factor, max_pool_stride, conv_stride)
    sep_conv = separable_conv2d(x_pool, [3, 3, int(x_pool.get_shape()[-1]), 1],
                                [1, 1, int(x_pool.get_shape()[-1]), num_filters], [1, conv_stride, conv_stride, 1],
                                name='sep_' + name)
    bn_sep_conv = tf.layers.batch_normalization(sep_conv, training=training_phase, name='bn_sep_conv_' + name)
    relu_sep_conv = tf.nn.relu(bn_sep_conv, name='relu_sep_conv' + name)
...
def transformation_block_v5(x, num_filters, constraint, training_phase, name):
    ...
    sep_conv1 = separable_conv2d(x_pool, [3, 3, int(x_pool.get_shape()[-1]), 1],
                                 [1, 1, int(x_pool.get_shape()[-1]), num_filters], [1, conv_stride, conv_stride, 1],
                                 name='sep_conv1_' + name)
    ...
    sep_conv2 = separable_conv2d(relu_sep_conv1, [3, 3, int(relu_sep_conv1.get_shape()[-1]), 1],
                                 [1, 1, int(relu_sep_conv1.get_shape()[-1]), num_filters], [1, 1, 1, 1],
                                 name='sep_conv2_' + name)
