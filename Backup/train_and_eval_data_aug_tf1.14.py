...
simpleLog = "/home/vrai/PycharmProjects/RE-ID/Log.txt"
...
for hp in set_params:
    ...
    iterator = tf.data.Iterator.from_structure(tf.compat.v1.data.get_output_types(tr_data.data), tf.compat.v1.data.get_output_shapes(tr_data.data))
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        '''
        x_rgb = tf.compat.v1.placeholder(tf.float32, [None, img_size[0], img_size[1], 3])
        x_depth = tf.compat.v1.placeholder(tf.float32, [None, img_size[0], img_size[1], 3])
        ...
        training_phase = tf.compat.v1.placeholder(tf.bool)
        loss_val = []
        loss_train = []
        acc_val = []
        acc_train = []
        ...
        # Initialize models
        with tf.compat.v1.variable_scope('rgb', reuse=None):
            model_rgb = ResNet(x_rgb, num_classes, mode='rgb')
        with tf.compat.v1.variable_scope('depth', reuse=None):
        ...
        # Conv1x1
        with tf.compat.v1.variable_scope('conv1x1'):
        ...
        trainable_variables_rnn = [v for v in tf.compat.v1.trainable_variables(scope="rnn")]
        trainable_variables_conv1x1 = [v for v in tf.compat.v1.trainable_variables(scope="conv1x1")]
        trainable_variables_rgb = [v for v in tf.compat.v1.trainable_variables(scope="rgb")]
        trainable_variables_depth = [v for v in tf.compat.v1.trainable_variables(scope="depth")]
        ...
        lr_values = [lr, lr / 10]
        decayed_lr = tf.compat.v1.train.piecewise_constant(global_step, lr_boundaries, lr_values)
        ...
        decayed_reg = tf.compat.v1.train.piecewise_constant(global_step, lr_boundaries, reg_values)
        recompute_reg = tf.compat.v1.assign(reg, sigm_reg)
        ...
        # train_step_rnn = tf.keras.optimizers.RMSprop(learning_rate=decayed_lr).get_updates(loss=loss,params=trainable_variables_rnn)
        # train_step_conv1x1 = tf.keras.optimizers.RMSprop(learning_rate=decayed_lr * lr_mult_conv1x1).get_updates(loss=loss,params=trainable_variables_conv1x1)
        train_step_rnn = RMSprop(lr=decayed_lr).get_updates(loss=loss,params=trainable_variables_rnn)

        train_step_conv1x1 = RMSprop(lr=decayed_lr * lr_mult_conv1x1).get_updates(loss=loss,params=trainable_variables_conv1x1)
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % 1 == 0:
                ...
                for i in range(val_batches_per_epoch):
                    ...
                    batch_loss, batch_acc, summary = sess.run([loss, accuracy, summary_op],
                                                                           feed_dict=feed_dict)
                ...
                if val_loss < best_loss_val:
                    best_saver.save(sess, "/media/vrai/ComputerVis/model")
        ...
        batchMatrix = defineImageRank1(full_pred, full_label)
