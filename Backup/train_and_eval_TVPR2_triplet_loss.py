...
# Data-related params
dataset_root_dir = "/content/drive/My Drive/TVPR2/train/"
test_root_dir = "/content/drive/My Drive/TVPR2/test/"
...
val_file = dataset_root_dir + '/val.txt'
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(tr_batches_per_epoch):
                ...
                if epoch == 0 and i == 0:
                    train_step_rnn = tf.keras.optimizers.RMSprop(lr=decayed_lr).get_updates(loss=tloss, params=trainable_variables_rnn)
                    train_step_conv1x1 = tf.keras.optimizers.RMSprop(lr=decayed_lr * lr_mult_conv1x1).get_updates(loss=tloss, params=trainable_variables_conv1x1)

                    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                    with tf.control_dependencies(update_ops):
                        train_step = tf.group(train_step_rnn, train_step_conv1x1, increment_global_step)

                batch_loss, _, summary, batch_acc_train, batch_preds = sess.run(
                    [tloss, train_step, summary_op, accuracy, preds],
                    feed_dict=feed_dict)
        ...
        for i in range(test_batches):
            ...
            batch_loss, batch_acc, summary, batch_preds = sess.run([TLoss, accuracy, summary_op, preds],
                                                                   feed_dict=feed_dict)
