...
dataset_root_dir = "D:/DATASET/tvpr2"
...
gallery_file = dataset_train_dir_rgb + '0_50id_50f_gallery5f/gallery.txt'
...
for hp in set_params:
    ...
    with tf.Session(config=config) as sess:
        ...
        clas = tf.placeholder(tf.int32, shape=[None])
        ...
        # Loss
        tloss = batch_hard_triplet_loss(clas, rnn_h, 0)  # margin 0.2

        loss_l2 = l2_rnn + l2_conv1x1  # + l2_rgb + l2_depth
        loss_cls = tf.reduce_mean(categorical_crossentropy(y, preds))

        loss = 0.001*loss_cls + 0.001*loss_l2 + tloss

        train_step_conv1x1 = RMSprop(lr=decayed_lr * lr_mult_conv1x1).get_updates(loss=loss,
                                                                                  params=trainable_variables_conv1x1)
        train_step_rnn = RMSprop(lr=decayed_lr).get_updates(loss=loss,
        ...
        catAcc = categorical_accuracy(y, preds)
        accuracy = tf.reduce_mean(catAcc)
        ...
        for i in range(tr_batches_per_epoch):
            ...
            classes = np.argmax(label_batch, axis=1)

            feed_dict = {x_rgb: rgb_batch, x_depth: depth_batch, y: label_batch, clas: classes, keep_prob: (1 - do),
        ...
        for i in range(val_batches_per_epoch):
            ...
            feed_dict = {x_rgb: rgb_batch, x_depth: depth_batch, y: label_batch, keep_prob: 1.0, clas: classes,
                         training_phase: False,
                         K.learning_phase(): 0}
            batch_loss, batch_acc = sess.run([loss, accuracy], feed_dict=feed_dict)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(tr_batches_per_epoch):
                ...
                feed_dict = {x_rgb: rgb_batch, x_depth: depth_batch, y: label_batch, keep_prob: (1 - do), clas: classes,
                             training_phase: True, K.learning_phase(): 1}

                batch_loss, _, batch_acc_train = sess.run([loss, train_step, accuracy], feed_dict=feed_dict)
        ...
        best_saver.restore(sess, pathToSavedModel)
        print("\nModel restored")
        ...
        for i in range(test_batches):
            ...
            [dist_matrix.append(
                euclidean_distance(np.array(img_feature), np.array(gallery_features), gallery_true_class, num_classes,
                                   frames_gallery)) for img_feature in batch_features]
